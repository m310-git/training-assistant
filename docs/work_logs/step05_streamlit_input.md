# Step 5: Streamlit入力画面

## 概要

Streamlitでトレーニング入力画面を構築する。最小限の動作確認を優先し、
BigQueryへの読み書きが正常に動作することを確認する。

## 前提条件

- [ ] Step 4 完了
- [ ] dbt run / dbt test が成功している
- [ ] secrets/sa-streamlit-app.json が存在する

---

## 手順

### 5-1. Streamlit依存パッケージインストール
...
#### streamlit/.streamlit/secrets.toml（Git管理外）
...
※ sa-streamlit-app.json の内容をコピーして貼り付ける
...
### 5-7. Gitコミット＆プッシュ
    ...
    git add streamlit/ docs/work_logs/
    git commit -m "feat: Streamlit input page with auth and auto-save"
    git push origin main

※ .streamlit/secrets.toml はpushしないこと
...
- [ ] Streamlit がインストールされている
- [ ] config.toml が作成されている
- [ ] secrets.toml が作成されている（Git管理外）
- [ ] BigQuery接続ユーティリティが動作する
- [ ] 認証（ログイン/ログアウト）が動作する
- [ ] 部位・種目の選択が動作する
- [ ] セット入力・自動保存が動作する
- [ ] 過去実績が表示される
