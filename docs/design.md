...
3. [技術スタックと選定理由](#3-技術スタックと選定理由)
   - 3.1. [Phase 1（MVP）構成](#31-phase-1mvp構成)
   - 3.2. [Phase 2〜5 追加コンポーネント](#32-phase-25-追加コンポーネント)
4. [システムアーキテクチャ](#4-システムアーキテクチャ)
   - 4.1. [Phase 1 アーキテクチャ図](#41-phase-1-アーキテクチャ図)
   - 4.2. [主要データフロー](#42-主要データフロー)
5. [データモデリング](#5-データモデリング)
   - 5.1. [レイクハウス3層構造](#51-レイクハウス3層構造)
   - 5.2. [ER図](#52-er図)
   - 5.3. [テーブル定義](#53-テーブル定義)
   - 5.4. [Mart層スキーマテスト定義](#54-mart層スキーマテスト定義)
6. [アプリケーション要件](#6-アプリケーション要件)
   - 6.1. [Streamlit アプリケーション画面構成](#61-streamlit-アプリケーション画面構成)
   - 6.2. [各画面の機能詳細](#62-各画面の機能詳細)
   - 6.3. [認証設計](#63-認証設計)
7. [通知設計](#7-通知設計)
   - 7.1. [通知一覧と負荷試算](#71-通知一覧と負荷試算)
   - 7.2. [無料枠対策](#72-無料枠対策)
   - 7.3. [LINE通知メッセージ](#73-line通知メッセージ)
   - 7.4. [Cloud Scheduler ジョブ設定](#74-cloud-scheduler-ジョブ設定)
8. [セキュリティと運用](#8-セキュリティと運用)
   - 8.1. [認証情報管理](#81-認証情報管理)
   - 8.2. [権限管理（最小権限の原則）](#82-権限管理最小権限の原則)
   - 8.3. [アプリケーションレベルの権限](#83-アプリケーションレベルの権限)
   - 8.4. [データ保護](#84-データ保護)
   - 8.5. [コスト監視](#85-コスト監視)
9. [エラーハンドリングと監視](#9-エラーハンドリングと監視)
   - 9.1. [エラーハンドリング戦略](#91-エラーハンドリング戦略)
   - 9.2. [ログ管理](#92-ログ管理)
   - 9.3. [通知カウンター（LINE無料枠管理）](#93-通知カウンターline無料枠管理)
10. [非機能要件](#10-非機能要件)
11. [テスト戦略](#11-テスト戦略)
    - 11.1. [テスト種別](#111-テスト種別)
    - 11.2. [dbt テスト一覧](#112-dbt-テスト一覧)
12. [デプロイ戦略](#12-デプロイ戦略)
    - 12.1. [CI/CDパイプライン](#121-cicdパイプライン)
    - 12.2. [ブランチ戦略](#122-ブランチ戦略)
13. [運用手順](#13-運用手順)
    - 13.1. [日次運用（自動）](#131-日次運用自動)
    - 13.2. [週次運用（自動）](#132-週次運用自動)
    - 13.3. [月次運用（自動）](#133-月次運用自動)
    - 13.4. [障害時の対応フロー](#134-障害時の対応フロー)
    - 13.5. [ユーザー追加手順](#135-ユーザー追加手順)
    - 13.6. [種目マスタ更新手順（管理者画面以外）](#136-種目マスタ更新手順管理者画面以外)
14. [将来の拡張計画](#14-将来の拡張計画)
    - 14.1. [Phase 2: データ品質](#141-phase-2-データ品質)
    - 14.2. [Phase 3: 自動化（Prefect Cloud）](#142-phase-3-自動化prefect-cloud)
